...
var jBlog = {
	...
	init:function(params){
		...
		$(window).bind("scroll",jBlog.scrollCallback);
		...
		$('.btn-explore').click(function(e){
			e.preventDefault();
			jBlog.ajaxGetExplore();
		});
		$('.btn-gotop').click(function(e){
			e.preventDefault();
			jBlog.goTop();
		});
		$('#btn-archive').on('click','a',function(e){
			e.preventDefault();
			if(jBlog.isLoad || jBlog.isEnd){
				return;
			}
			jBlog.isLoad = true;
			jBlog.ajaxLoadArchive();
		});
		jBlog.syncSize();
	},
	initLike:function(){	//喜欢按钮
	...
	ajaxLoadArchive:function(){
		var btn = $('#btn-archive').find('.btn');
		...
		$.get(url,function(rs){
			var main = $(rs).find('.post');
			var page = $(rs).find('.btn');
			$(main).appendTo($('.main-inner'));
			if(page.find('a').length>0){
				btn.html(page.html());
			}else{
				btn.html('没有更多内容了').addClass('nomore');
			}
